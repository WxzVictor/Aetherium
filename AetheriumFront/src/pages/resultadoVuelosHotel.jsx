        ...
        <div className="combo-card" key={idx}>
            <div className="combo-vuelo">
                ...
                {combo.vuelos.map((vuelo, i) => (
                    <div className="card" key={i}>
                        ...
                        <div>
                            Salida: {formatDate(vuelo.departureTime)} {new Date(vuelo.departureTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                        </div>
                        <div>
                            Llegada: {formatDate(vuelo.arrivalTime)} {new Date(vuelo.arrivalTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                        </div>
                        <div>
                            Precio vuelo: {(vuelo.price / 100).toFixed(2)} €
                        </div>
                    </div>
                ))}
            </div>
            <div className="combo-hotel">
                ...
                <div className="card hotel-card">
                    ...
                    <div>Ubicación: {combo.hotel.city}, {combo.hotel.country}</div>
                    <div>Dirección: {combo.hotel.address}</div>
                    <div>
                        Estrellas: {"★".repeat(Math.min(5, Math.floor(combo.hotel.rating / 10))).padEnd(5, "☆")}
                    </div>
                    <div>Precio por noche: {(combo.hotel.pricePerNight / 100).toFixed(2)} €</div>
            ...
            <div className="combo-precio precio">
                <div className="monto">
                    Total vuelo + hotel: {((combo.vuelos.reduce((acc, v) => acc + v.price, 0) / 100) + (combo.hotel.pricePerNight / 100)).toFixed(2)} €
                </div>
                <button
                    className="btn-rV"
                    onClick={() => {
            ...
            <div className="login-page">
                ...
                <div className="contenido-visible">
                    <div className="resultado-vuelos-container">
                        ...
                        {loading ? (
                            <p className="cargando" data-text="Cargando resultados...">Cargando resultados...</p>
                        ) : (
